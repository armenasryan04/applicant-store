...
public class HhConversionUtils {
    ...
    public XWPFTableRow findNextTitleRow(List<XWPFTableRow> rows, XWPFTableRow startRow) {
        return rows.stream()
                .filter(row -> rows.indexOf(row) > rows.indexOf(startRow) && row.getTableCells().size() == 1)
                .filter(row -> row.getCell(0).getParagraphs().stream().noneMatch(paragraph -> paragraph.getRuns().stream().noneMatch(run -> Objects.equals(run.getColor(), "AEAEAE"))))
                .findFirst()
                .orElse(null);
    }
    ...
    public Optional<XWPFTableRow> findTitleRow(List<XWPFTableRow> rows, String title) {
        return rows.stream().filter(row ->
                row.getTableCells().stream().anyMatch(cell -> cell.getText().contains(title))
        ).findFirst();
    }

    public List<XWPFParagraph> findParagraphsFromTo(List<XWPFParagraph> paragraphs, XWPFParagraph start, XWPFParagraph finish) {
        if (finish == null) {
    ...
    public List<XWPFParagraph> findParagraphsBetween(List<XWPFParagraph> paragraphs, XWPFParagraph start, XWPFParagraph finish) {
    ...
    public List<XWPFTableRow> findRowsFromToTitleRows(List<XWPFTableRow> rows, XWPFTableRow start, XWPFTableRow finish) {
        if (finish == null) {
    ...
    public List<XWPFTableRow> findRowsBetween(List<XWPFTableRow> rows, XWPFTableRow start, XWPFTableRow finish) {
